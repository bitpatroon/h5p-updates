...
class UpdatedH5PContentValidator extends \H5PContentValidator
{
    public $h5pF;
    public $h5pC;
    ...
    public function validateText(&$text, $semantics)
    {
        ...
        if (isset($semantics->maxLength)) {
            if (!extension_loaded('mbstring')) {
                $this->h5pF->setErrorMessage($this->h5pF->t('The mbstring PHP extension is not loaded. H5P need this to function properly'),
        ...
        if (!($text === '' && isset($semantics->optional) && $semantics->optional) && isset($semantics->regexp)) {
            ...
            if (preg_match($pattern, $text) === 0) {
                ...
                $this->h5pF->setErrorMessage($this->h5pF->t('Provided string is not valid according to regexp in semantics. (value: "%value", regexp: "%regexp")',
                    array('%value' => $text, '%regexp' => $pattern)), 'semantics-invalid-according-regexp');
                $text = '';
            }
        }
    }
                ...
                if (!extension_loaded('mbstring')) {
                    ...
                    if (!preg_match($wl_regex, mb_strtolower($file))) {
                        $this->h5pF->setErrorMessage($this->h5pF->t('File "%filename" not allowed. Only files with the following extensions are allowed: %files-allowed.',
                            array('%filename' => $file, '%files-allowed' => $whitelist)), 'not-in-whitelist');
                        $valid = false;
                    }
                }
            }
            ...
            foreach ($group as $key => &$value) {
                ...
                if ($found) {
                    if ($function) {
                        ...
                        // known validator.
                        $this->h5pF->setErrorMessage($this->h5pF->t('H5P internal error: unknown content type "@type" in semantics. Removing content!',
                            array('@type' => $field->type)), 'semantics-unknown-type');
                        unset($group->$key);
                    }
                } else {
