...
class UpdatedH5PValidator extends \H5PValidator
{
    public $h5pF;
    public $h5pC;
        ...
        if (is_dir($languagePath)) {
            ...
            foreach ($languageFiles as $languageFile) {
                ...
                if (preg_match('/^(-?[a-z]+){1,7}\.json$/i', $languageFile) === 0) {
                    $this->h5pF->setErrorMessage($this->h5pF->t('Invalid language file %file in library %library',
                        array('%file' => $languageFile, '%library' => $file)), 'invalid-language-file');
                    return false;
                }
                $languageJson = $this->getJsonData($languagePath . '/' . $languageFile, true);
                if ($languageJson === false) {
                    $this->h5pF->setErrorMessage($this->h5pF->t('Invalid language file %languageFile has been included in the library %name',
                        array('%languageFile' => $languageFile, '%name' => $file)), 'invalid-language-file');
                    return false;
                }
    ...
    private function isValidRequirement($h5pData, $requirement, $library_name, $property_name)
    {
        ...
        if (is_string($requirement)) {
            if ($requirement == 'boolean') {
                if (!is_bool($h5pData)) {
                    $this->h5pF->setErrorMessage($this->h5pF->t("Invalid data provided for %property in %library. Boolean expected.",
                        array('%property' => $property_name, '%library' => $library_name)));
                    $valid = false;
                }
            } else {
                ...
                if (is_string($h5pData) || is_int($h5pData)) {
                    if (preg_match($requirement, $h5pData) === 0) {
                        $this->h5pF->setErrorMessage($this->h5pF->t("Invalid data provided for %property in %library",
                            array('%property' => $property_name, '%library' => $library_name)));
                        $valid = false;
                    }
                } else {
            ...
            $this->h5pF->setErrorMessage($this->h5pF->t("Can't read the property %property in %library",
                array('%property' => $property_name, '%library' => $library_name)));
            $valid = false;
        }
        return $valid;
    ...
    private function isValidRequiredH5pData($h5pData, $requirements, $library_name)
    {
        ...
        foreach ($requirements as $required => $requirement) {
            ...
            if (isset($h5pData[$required])) {
                ...
                $this->h5pF->setErrorMessage($this->h5pF->t('The required property %property is missing from %library',
                    array('%property' => $required, '%library' => $library_name)), 'missing-required-property');
                $valid = false;
            }
        }
        return $valid;
